...
class JsonSourceOutput extends SourceOutput {
   ...
   * @return {Object|Boolean} True or params if next input stream exists
   */
  ...
  _handleParserComplete(json) {
    ...
    const nextInputStreamParams = this._configureNextInputStream(json);
    if (!nextInputStreamParams) {
      return;
    }
    if (_.isObject(nextInputStreamParams)) {
      this._params = Object.assign({}, this._initialParams, nextInputStreamParams);
    }
